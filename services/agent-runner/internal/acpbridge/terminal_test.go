...
func TestVerifyTicket_RoundTrip(t *testing.T) {
	...
	ticket := mintTicket(secret, envID, time.Now().Add(time.Minute))
	if err := verifyTicket(secret, ticket, envID); err != nil {
...
func TestVerifyTicket_WrongSecret(t *testing.T) {
	envID := uuid.New()
	ticket := mintTicket([]byte("secret-a"), envID, time.Now().Add(time.Minute))
	if err := verifyTicket([]byte("secret-b"), ticket, envID); err == nil {
...
func TestVerifyTicket_EnvironmentIDMismatch(t *testing.T) {
	secret := []byte("shared-internal-api-key")
	ticket := mintTicket(secret, uuid.New(), time.Now().Add(time.Minute))
	if err := verifyTicket(secret, ticket, uuid.New()); err == nil {
...
func TestVerifyTicket_MalformedTicket(t *testing.T) {
	secret := []byte("shared-internal-api-key")
	if err := verifyTicket(secret, "not-a-valid-ticket!!!", uuid.New()); err == nil {
		t.Fatal("expected an error for a malformed ticket, got nil")
	}
	if err := verifyTicket(secret, "", uuid.New()); err == nil {
