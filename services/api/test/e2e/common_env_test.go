...
func newE2EEnv(t *testing.T) *e2eEnv {
	...
	engine := router.New(router.Deps{
		...
		User:                 handler.NewUserHandler(userService).WithAuthorizer(authz.NewAuthorizer(authzStore)).WithRoleLookup(roleRepo),
		...
		Agent:                handler.NewAgentHandler(agentService, "", "", "").WithMemberRepo(projectRepo).WithAuthorizer(authz.NewAuthorizer(authzStore)).WithGlobalRoleLookup(globalRoleService),
