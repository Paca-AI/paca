#!/usr/bin/env bash
# Install/update the com.galaxy.analytics plugin on the PROD galaxy-paca stack.
#
...
if [[ -n "$UUID" && "$UUID" != "null" ]]; then
  ...
  [[ "$HTTP" == "200" ]] || { cat "$RESP"; rm -f "$RESP"; fail "update failed (HTTP $HTTP)"; }
else
  ...
  [[ "$HTTP" == "201" ]] || { cat "$RESP"; rm -f "$RESP"; fail "install failed (HTTP $HTTP)"; }
fi
rm -f "$RESP"
...
Done. In the SPA (allow ~5 min for the plugins query cache, or hard-reload):
  - project sidebar shows the "Analytics" nav item (chart icon)
  - /projects/<id>/plugins/$PLUGIN_ID/analytics renders the dashboard
  - "+ Add view" on any board offers the Analytics plugin layout
Charts are computed from the caller's own session against /api/v1 — if every
panel shows an auth error, the Paca session expired: reload the SPA to refresh
the cookie, then reopen the page. See README.md.
EOF
