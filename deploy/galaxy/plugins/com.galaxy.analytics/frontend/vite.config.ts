...
export default defineConfig({
	plugins: [
		federation({
			name: "com_galaxy_analytics",
			// Emitted inside assets/ so plugin.json's
			// /plugins/com.galaxy.analytics/assets/remoteEntry.js matches the
			// store convention documented in deploy/caddy/Caddyfile.
			filename: "assets/remoteEntry.js",
			manifest: false,
			dts: false,
			exposes: {
				"./AnalyticsView": "./src/AnalyticsView.tsx",
			},
			shared: {
	...
	build: {
		...
		// /plugins/com.galaxy.analytics/ and plugin.json points at
		// /plugins/com.galaxy.analytics/assets/remoteEntry.js.
		emptyOutDir: true,
	},
});
