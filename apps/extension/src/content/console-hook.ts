...
//
// Gated on the same cheap, synchronous paca_port cookie check the isolated
// content script uses before doing anything (see content/index.ts) —
// without this, every page on the entire internet would have its
// console.error/console.warn silently overridden and its errors relayed
// via postMessage, whether or not it's ever going to be a Paca preview.
import { readPacaPort } from "../shared/paca-port";

if (readPacaPort() !== null) {
	installConsoleHook();
}
